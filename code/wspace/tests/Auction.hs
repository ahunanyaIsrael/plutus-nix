...
--             traceIfFalse "Bid too low" (bid > highestBid dat)
--         CloseAuction ->
...
-- main :: IO ()
-- main = do
--     putStrLn "Compiled English Auction smart contract!"
--     writeValidator "./assets/auction.plutus" validator
  ...
  where
    ...
    sellerPaid =
        let prevBid = highestBid dat
         in if prevBid <= 0
               then True
               else payToPubKey (seller dat) prevBid info
...
main = do
    ...
    writeValidator "./assets/auction5.plutus" validator
